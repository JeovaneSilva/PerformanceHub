...
try {
    ...
    if (empty($ids)) {
        throw new Exception("Nenhuma avaliacao selecionada.");
    }

    $sql = "DELETE FROM tbAvaliacao WHERE avaliacao_id = ?";
    ...
    if ($sucesso_count > 0) {
        $retorno = [
            'status' => 'success',
            'message' => $sucesso_count . ' avaliacao(oes) excluida(s) com sucesso.'
        ];
    } else {
